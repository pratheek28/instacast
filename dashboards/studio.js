...
const SDashBoard = () => {
  ...
  useEffect(() => {
    ...
    const handleSubmit = () => {
      fetch('http://127.0.0.1:8000/getStudioDash', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ data }), // Send data as an object with topic
      })
    ...
    const handleSubmit2 = () => {
      fetch('http://127.0.0.1:8000/getRoles', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
<div style="text-align: center;">
  <button type="submit" style="margin-top: 10px; padding: 8px 12px; border: none; border-radius: 5px; background-color: #28a745; color: white; cursor: pointer;">Create</button>
    ...
    $('#roleForm').on('submit', function (e) {
      ...
      const handleSubmit = () => {
        fetch('http://127.0.0.1:8000/putNewRoles', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
